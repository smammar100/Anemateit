...
> **Recreate this exact component.** Paste this entire prompt into any AI coding tool (Claude Code, Cursor, v0, Lovable, Bolt, Kimi…) — it's self-contained, with the full source included below. **Stack:** React 18+ + TypeScript, SVG + `requestAnimationFrame` (swap the `className`s for inline styles if you're not using Tailwind). **Dependencies:** none.

The **"AMMAR"** wordmark as a row of five coloured Lissajous curves — the **cursor.com/compile** hero engine re-parameterised to spell a name. Every letter is built from one or more *pure* Lissajous strokes, `x = cx + A·sin(a·t + δ)`, `y = cy + B·sin(b·t)`, whose frequency ratio, phase and rotation are chosen so the raw mathematical curve **evokes** the letter:

- **A** — a degenerate 2:1 parabola at δ=1.42, opened into a narrow lens: an italic A-frame. The two A's mirror each other's tilt (−95° / −85°).
- **M** — the classic 1:2 lemniscate (sideways figure-8) whose two lobes read as M's twin peaks.
- **R** — built from existing recipes only, so it sits in the family: two **"I"** strokes (a 1:1 δ=0 line — perfectly straight) for the **stem** (stood vertical) and the **leg** (kicking down-right from the mid-junction), plus the **"A"** lens (2:1 δ=1.42) tilted −15° and hung off the upper stem as the **bowl**. All three are sized ≈1.25× the bare recipes so the R fills its cell like the A and M.

## How it works

- **Letters as cells.** Each glyph lives in a 200×200 cell; cells overlap via negative `gapBefore` so the curves interleave across the row. Amplitude is 90% of the half-cell, scaled per-stroke (`scaleX`/`scaleY`); each stroke is rotated about the cell centre and offset within it.
- **Strokes per cell.** A letter carries a `strokes[]` array — the A and M have one; the R has three (stem / bowl / leg). Every stroke in a cell shares the letter's phase + rotation animation and colour.
- **Intro = phase rewind.** Each letter mounts with its phase offset by `−amount` (−π for A/M; π/2 for the R so its straight stems stay legible) and eases to 0 over ~2s, staggered left → right (delays 0–0.4s). The A's rotate in ∓30° and the R swings in −12°. While the phase eases, the curves morph through every intermediate Lissajous state — the signature "settling" effect (the R's straight 1:1 stems sweep in from ellipses).
- **Hover = phase drift.** Each letter has its own transparent 200×200 hit-rect. While hovered, its phase drifts `+0.3π` over 4s (easeOutQuart) and *stays* there; on leave it eases back. On the R the straight stems bow into ellipses as their δ drifts off 0. Mid-transition reversals capture current progress, so rapid pointer moves stay smooth.
...
- To spell a different word, edit the `LETTERS` array — give each letter a `strokes[]` whose `(a, b, delta, rotation)` gesture at the glyph, set colours and overlaps.
...
const LETTERS: Letter[] = [
  {
    rotation: -95, stroke: '#F76D18', strokes: [A_FIGURE],
    intro: {
  ...
  {
    rotation: 0, stroke: '#2C9F28', gapBefore: -50, strokes: [M_FIGURE],
    intro: { phase: { amount: Math.PI, duration: 2, stagger: 0, delay: 0.1, easing: 'easeInOut' } },
  },
  {
    rotation: 0, stroke: '#8C89E7', gapBefore: -25, strokes: [M_FIGURE],
    intro: { phase: { amount: Math.PI, duration: 2, stagger: 0, delay: 0.2, easing: 'easeInOut' } },
  },
  {
    rotation: -85, stroke: 'currentColor', gapBefore: -30, strokes: [A_FIGURE],
    intro: {
  ...
  },
  {
    ...
    rotation: 0, stroke: '#2268FF', gapBefore: -105,
    strokes: [
...
A study of the **cursor.com/compile** event hero — a word drawn as pure Lissajous curves with phase-spin intro and per-letter hover drift — re-parameterised to spell "AMMAR". Lissajous technique reference: CodePen **"Lissajous"** by **jake** (jak_e): <https://codepen.io/jak_e/pen/ZvwgOg>.
