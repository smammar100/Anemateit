    ...
    >
      ...
      {!compact && (
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="flex items-center rounded-full border border-base-100 bg-white/60 backdrop-blur shadow-sm gap-2.5 px-3 py-2">
            {variants.map((v) => (
              <button
                key={v.name}
                type="button"
                ...
                className={cn(
                  "rounded-full outline-none ring-offset-2 ring-offset-white transition-all duration-200 cursor-pointer size-6",
                  variant === v.name
