...
/**
 * AI Sphere — a true-3D living orb
 *
 * A real Three.js sphere — a glass fresnel shell wrapping volumetric FBM gas —
 * that genuinely rotates in 3D, with the original "siri" flower-bloom anchored
 * at its centre and 5 color states.
 *
 * Plasma sphere adapted from a CodePen by sabosugi
 * (https://codepen.io/sabosugi/pen/jErWrMe); the centre flower-bloom is the
 * inner-bloom shape from a CodePen by Reza Saboori Amleshi
 * (https://codepen.io/Reza-Saboori-Amleshi/pen/bGXyLdo).
 */
...
type SceneRefs = {
  plasmaMat: THREE.ShaderMaterial;
...
export default function AISphere({
  ...
  useEffect(() => {
    ...
    // ── Centre bloom: the siri flower-burst on a camera-facing billboard,
    //    pinned at the centre so it stays put while the sphere rotates. ────
    ...
    // ── True 3D rotation: drag spins the whole sphere, with inertia ──────
    let dragging = false;
