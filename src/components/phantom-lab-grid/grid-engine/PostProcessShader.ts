...
export class CustomPostProcessShader {
   ...
   * @param gl - WebGL context from OGL Renderer
   * @param mainRenderer - OGL Renderer instance (used to draw the post-process pass)
   * @param initialParams - Optional initial parameters for the shader effects
   */
  constructor(
    ...
    // Use safer default values to ensure the screen is visible
    this._distortionIntensity = initialParams.distortionIntensity ?? 0;
    this._vignetteOffset = initialParams.vignetteOffset ?? 1.2; // Moved outside screen to disable vignette by default
  ...
  resize(width: number, height: number): void {
    ...
    this.updateUniforms(); // Update aspect ratio dependent uniforms
  }
